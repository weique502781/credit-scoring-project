...
#     # ===============================
#     try:
#         ensemble_bundle = load_models("models/saved_models")
#         if "final_ensemble" in ensemble_bundle:
#             final_ensemble = ensemble_bundle["final_ensemble"]
#             print("✓ 已加载最终集成模型 final_ensemble")
#             print(f"  使用的最优阈值: {final_ensemble.threshold:.3f}")
#         else:
#             final_ensemble = None
#             print("⚠ 未找到 final_ensemble 模型")
#     except Exception as e:
#         print(f"✗ 加载最终集成模型失败: {e}")
#         final_ensemble = None
#
#     # 6. 模型评估与可视化
#     print("\n模型评估与可视化...")
...
#
#     # 2️⃣ 最终集成模型 ROC（加权投票概率，不受阈值影响）
#     if final_ensemble is not None:
#         try:
#             y_prob_ens = final_ensemble.predict_proba(X_test_selected.values)[:, 1]
#             roc_analyzer.add_model("final_ensemble", y_test, y_prob_ens)
#             print("✓ final_ensemble ROC 曲线已加入")
#         except Exception as e:
#             print(f"✗ final_ensemble ROC分析失败: {e}")
#
#     # 3️⃣ 绘制 ROC 曲线
#     try:
#         roc_analyzer.plot_roc_curves(
#             save_path='reports/figures/roc_curves.png',
#             title='各模型 ROC 曲线对比（含最终集成模型）'
#         )
#         print("✓ ROC曲线图已保存")
#     except Exception as e:
#         print(f"✗ ROC曲线生成失败: {e}")
